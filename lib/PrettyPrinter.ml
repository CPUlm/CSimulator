...
let print_env print lp sep rp ff env =
  ...
  Env.iter
    (fun x ty ->
      if !first then (
        first := false ;
        fprintf ff "%a" print (x, ty) )
      else fprintf ff "%s%a" sep print (x, ty) )
    env ;
  fprintf ff "%s" rp
...
let print_ty ff ty =
  match ty with TBit -> () | TBitArray n -> fprintf ff " : %d" n
...
let print_arg ff arg =
  match arg with Aconst v -> print_value ff v | Avar id -> fprintf ff "%s" id
...
let print_exp ff e =
  ...
  | Erom (addr, word, ra) ->
      fprintf ff "ROM %d %d %a" addr word print_arg ra
  | Eram (addr, word, ra, we, wa, data) ->
      fprintf ff "RAM %d %d %a %a %a %a" addr word print_arg ra print_arg we
  ...
  | Eslice (min, max, x) ->
      fprintf ff "SLICE %d %d %a" min max print_arg x

let print_eq ff (x, e) = fprintf ff "%s = %a@." x print_exp e

let print_var ff (x, ty) = fprintf ff "@[%s%a@]" x print_ty ty

let print_vars ff env =
  fprintf ff "@[<v 2>VAR@,%a@]@.IN@," (print_env print_var "" ", " "") env
