...
let to_c p nbSteps =
  let regLst = ref [] in
  List.iter
    (fun (_, exp) -> match exp with Ereg a -> regLst := a :: !regLst | _ -> ())
    p.p_eqs ;
  ...
  List.iter
    (fun (ident, exp) ->
      match exp with
      | Eram _ | Erom _ ->
          memDef := !memDef ^ "Memory " ^ memPrefix ^ ident ^ ";\n"
      | _ ->
          () )
    p.p_eqs ;
  ...
  Env.iter
    (fun ident _ ->
      if isReg ident then
        gateDef := !gateDef ^ "GateReg " ^ varPrefix ^ ident ^ " (true);\n"
      else gateDef := !gateDef ^ "Gate " ^ varPrefix ^ ident ^ " (true);\n" )
    p.p_vars ;
  let v2 = search_replace v1Bis "$GATE_DEF$" !gateDef in
  let from_arg = function
    | Avar ident ->
        varPrefix ^ ident ^ ".calcGate()"
    | Aconst (VBit b) ->
        if b then "1" else "0"
    | Aconst (VBitArray a) ->
        "0b"
        ^ Array.fold_left (fun s b -> if b then "1" ^ s else "0" ^ s) "" a
        ^ "u"
  in
  let gateSize ident =
    ...
    (fun (ident, exp) ->
      ...
      let exp_to_string ident = function
        | Earg a1 ->
            "return " ^ from_arg a1 ^ ";"
        | Ebinop (And, a1, a2) ->
            "return (" ^ from_arg a1 ^ " & " ^ from_arg a2 ^ ");"
        | Ebinop (Or, a1, a2) ->
            "return (" ^ from_arg a1 ^ " | " ^ from_arg a2 ^ ");"
        | Ebinop (Xor, a1, a2) ->
            "return (" ^ from_arg a1 ^ " ^ " ^ from_arg a2 ^ ");"
        ...
        | Enot a1 ->
            "return ~" ^ from_arg a1 ^ ";"
        | Eselect (i, a1) ->
            "return (" ^ from_arg a1 ^ " >> " ^ string_of_int i ^ ");"
        | Ereg id ->
            "return " ^ varPrefix ^ id ^ ".getOldValue();"
        | Eram (_, _, readAddr, _, _, _) | Erom (_, _, readAddr) ->
            "return " ^ memPrefix ^ ident ^ ".get(" ^ from_arg readAddr ^ ");"
        | Emux (a1, a2, a3) ->
            "return ( (" ^ from_arg a1 ^ ")? " ^ from_arg a3 ^ " : "
            ^ from_arg a2 ^ ");"
        | Eslice (i1, _, a1) ->
            "return (" ^ from_arg a1 ^ " >> " ^ string_of_int i1 ^ ");"
        | Econcat (a1, a2) ->
            let s1 =
              match a1 with
              | Avar ident ->
              ...
              | Aconst (VBitArray ar) ->
                  Array.length ar
            in
            "return ((" ^ from_arg a2 ^ " << " ^ string_of_int s1 ^ ") | ("
            ^ from_arg a1 ^ " & " ^ mask1 s1 ^ "));"
      in
      fctDef :=
        !fctDef ^ "uint32_t " ^ fctPrefix ^ ident ^ " (){"
        ^ exp_to_string ident exp ^ "}\n" )
    p.p_eqs ;
  ...
  List.iter
    (fun (ident, _) ->
      fctSet :=
        !fctSet ^ "\t" ^ varPrefix ^ ident ^ ".setFct(&" ^ fctPrefix ^ ident
        ^ ");\n" )
    p.p_eqs ;
  ...
  List.iter
    (fun (ident, exp) ->
      match exp with
      | Erom (addrSize, wordSize, _) ->
          readRom :=
            !readRom ^ "\t" ^ memPrefix ^ ident ^ ".initRom("
            ^ string_of_int addrSize ^ "," ^ string_of_int wordSize ^ ");\n"
      | _ ->
          () )
    p.p_eqs ;
  ...
  let init = ref "" in
  List.iter
    (fun (ident, _) -> init := !init ^ "\t\t" ^ varPrefix ^ ident ^ ".init();\n")
    p.p_eqs ;
  ...
  List.iter
    (fun ident ->
      input :=
        !input ^ "\t\t" ^ varPrefix ^ ident ^ ".readInput("
        ^ string_of_int (gateSize ident)
        ^ ");\n" )
    p.p_inputs ;
  ...
  List.iter
    (fun (ident, _) ->
      calc :=
        !calc ^ "\t\tif(" ^ varPrefix ^ ident ^ ".mustBeCalculated) "
        ^ varPrefix ^ ident ^ ".calcGate();\n" )
    p.p_eqs ;
  ...
  List.iter
    (fun (ident, exp) ->
      ...
      | Eram (_, _, _, we, wa, data) ->
          writeRam :=
            !writeRam ^ "if (" ^ from_arg we ^ ") " ^ memPrefix ^ ident
            ^ ".set(" ^ from_arg wa ^ ", " ^ from_arg data ^ ");\n"
      | _ ->
          () )
    p.p_eqs ;
  ...
  List.iter
    (fun ident ->
      output :=
        !output ^ "\t\tprintf(\"=> " ^ ident ^ " = \"); " ^ varPrefix ^ ident
        ^ ".show("
        ^ string_of_int (gateSize ident)
        ^ "); printf(\"\\n\");\n" )
    p.p_outputs ;
